package lc.daily.date2209.Q1592;
...
class Solution {
    public String reorderSpaces(String text) {
        ...
        int gapSpace = countSpace / (Math.max(countWords - 1,1));
        StringBuilder sb = new StringBuilder();
        for (String word : words) {
            sb.append(word);
            if (countSpace >= gapSpace){
                sb.append(" ".repeat(gapSpace));
                countSpace -= gapSpace;
            }
        }
        sb.append(" ".repeat(countSpace));
        return sb.toString();
    }
}
public class Main {
